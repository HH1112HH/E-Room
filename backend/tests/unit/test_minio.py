...
class TestMinioCRUDInit:
    def test_ensure_bucket_uses_settings_bucket_when_empty(self) -> None:
        ...
        mock_client.make_bucket.assert_called_once_with(settings.minio_bucket)

    def test_ensure_bucket_uses_custom_bucket_name(self) -> None:
        mock_client = MagicMock()
        mock_client.bucket_exists.return_value = False
...
class TestPresignedUrls:
    def test_presigned_get_url_calls_client_with_timedelta(self) -> None:
        ...
        mock_client.presigned_get_object.assert_called_once_with("test-bucket", "my-object.txt", expires=timedelta(seconds=1800))
    ...
    def test_presigned_put_url_calls_client_with_timedelta(self) -> None:
        ...
        mock_client.presigned_put_object.assert_called_once_with("test-bucket", "upload.dat", expires=timedelta(seconds=900))

    def test_presigned_get_url_default_expiry(self) -> None:
        mock_client = MagicMock()
        crud = MinioCRUD(client=mock_client, bucket="test-bucket")
        crud.presigned_get_url("obj.bin")
        mock_client.presigned_get_object.assert_called_once_with("test-bucket", "obj.bin", expires=timedelta(seconds=3600))
