...
class NotificationService(CRUDRepository):
    ...
    def list_for_user(self, user_id: UUID, skip: int = 0, limit: int = 20) -> list[Notification]:
        return self.get_many(self.session, user_id=user_id, order_by="created_at", desc=True, skip=skip, limit=limit)

    def count_unread(self, user_id: UUID) -> int:
        items = self.get_many(self.session, user_id=user_id, order_by="created_at", desc=True)
